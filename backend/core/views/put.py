...
class TransferTeamOwnership(APIView):
    ...
    def put(self, request, team_id):
        ...
        # Ensure new owner is a team member and at least LS (LS/LDC/CTC/ADMIN) by IR access level
        is_team_member = TeamMember.objects.filter(
            team=team,
            ir=new_owner,
        ).exists()
        is_eligible_level = new_owner.ir_access_level <= AccessLevel.LS

        if not is_team_member or not is_eligible_level:
            return Response(
                {"detail": "New owner must be a team member with access level LS or above"},
                status=status.HTTP_400_BAD_REQUEST
            )
